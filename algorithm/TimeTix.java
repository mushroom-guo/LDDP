...
public class TimeTix {
    ...
    public double linerProgram(Solution solution) throws Exception {
        ...
        //最早开始工作时间约束
        for (Drone drone : solution.drones) {
            if (drone.arcs.size() > 0) {
                // 获取该无人机的第一个动作
                Arc firstArc = drone.arcs.get(0);
                GRBLinExpr startConstr = new GRBLinExpr();
                startConstr.addTerm(1.0, firstArc.start);
                // 约束：第一个动作的开始时间 >= Para.startTime (480)
                model.addConstr(startConstr, GRB.GREATER_EQUAL, para.startTime, "StartAtWorkingTime");
            }
        }
        ...
        for (Drone drone : solution.drones) {
            for (Arc arc : drone.arcs) {
                ...
                model.addConstr(constr1, GRB.EQUAL, drone.preTime, "constr1");
            }
        }
        ...
        for (Locker locker : problem.lockers) {
            if (locker.arcList.size() > 1) {
                for (int i = 0; i < locker.arcList.size() - 1; i++) {
                    ...
                    model.addConstr(constr3, GRB.GREATER_EQUAL, 0.1, "constr3");
                }
            }
        }
...
//            System.out.println("Feasible, Total Time Window Violation Cost = " + objVal);
            for (Drone drone : solution.drones) {
                for (Arc arc : drone.arcs) {
                    // 1. 从 Gurobi 变量中获取优化后的值 (.get(GRB.DoubleAttr.X))
                    // 2. 赋值给 Arc 对象的属性
                    arc.startTime = arc.start.get(GRB.DoubleAttr.X);
                    arc.endTime = arc.end.get(GRB.DoubleAttr.X);
                }
            }
        }else {
